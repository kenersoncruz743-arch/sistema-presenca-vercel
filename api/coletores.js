  ...
  try {
    if (req.method === 'POST') {
      ...
      switch (action) {
        case 'salvarRegistro':
          try {
            const result = await sheetsService.salvarRegistro(data);
        ...
        case 'obterStatusCompleto':
          try {
            const [status, resumo] = await Promise.all([
              sheetsService.obterStatus(),
              sheetsService.gerarResumo()
            ]);
    ...
    if (req.method === 'GET') {
      ...
      if (action === 'obterDados') {
        const dados = await sheetsService.obterDadosColaboradores();
