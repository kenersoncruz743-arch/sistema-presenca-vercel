...
class SheetsColetorService {
  ...
  async salvarRegistroChave(chapa, nome, funcao, numeroChave, tipoOperacao, situacoes) {
    try {
      ...
      // ===== 1. SALVA NO HISTÓRICO DE CHAVES (GOOGLE_SHEETS_ID_COLETOR - ABA "Historico_chaves") =====
      try {
      ...
      // ===== 2. EMPILHA NA ABA CHAVES (GOOGLE_SHEETS_ID - EMPILHAMENTO) =====
      try {
  ...
  async salvarNoHistoricoChaves(dados) {
    try {
      ...
      await sheetHistorico.addRow({
        ...
        'Nome': dados.nome,
        'Funcao': dados.funcao,
        'NumeroChave': String(dados.numeroChave),
        'TipoOperacao': dados.tipoOperacao,
        'Situacao': dados.situacao,
        'Supervisor': dados.supervisor,
        'Turno': dados.turno
      ...
      console.log('[SHEETS_CHAVE] ✓ Adicionado ao histórico');
  ...
  async empilharNaAbaChaves(dados) {
    try {
      ...
      for (const row of rows) {
        ...
        if (dataRow === dados.data && 
            chapaRow === dados.chapa && 
            chaveRow === String(dados.numeroChave)) {
          registroExistente = row;
      ...
      if (registroExistente) {
        ...
        try {
          ...
          console.log('[SHEETS_CHAVE] ✓ Registro empilhado');
        } catch (addError) {
