// lib/sheets_2.js - VERSÃO CORRIGIDA: CORRIGE SALVAMENTO NA PLANILHA ATUAL
  ...
  async salvarRegistro(chapa, nome, funcao, numeroColetor, tipoOperacao, situacoes) {
    try {
      ...
      try {
        ...
        // Continua mesmo se falhar na planilha atual
  ...
  async salvarNoHistorico(dados) {
    try {
      ...
      if (rows.length > 0) {
        ...
        const ultimoTipo = String(ultima.get('TipoOpe') || '').trim();
    ...
    try {
      ...
      if (!sheetAtual) {
        ...
        try {
          ...
          console.log('[SHEETS_COLETOR] ✓ Aba criada');
        } catch (createError) {
      ...
      if (registroExistente) {
        ...
        try {
          ...
          console.log('[SHEETS_COLETOR] ✓ Registro atualizado');
        } catch (updateError) {
        ...
        try {
          ...
          console.log('[SHEETS_COLETOR] ✓ Novo registro criado');
        } catch (addError) {
  ...
  async obterColetorStatus() {
    try {
      ...
      rows.forEach(row => {
        ...
        const supervisor = String(row.get('Supervisor') || '').trim();
        const ultima = String(row.get('UltimaAtualizacao') || '').trim();
