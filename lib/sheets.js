      ...
      for (const row of rows) {
        try {
          const item = {
            ...
            visitasPendente: String(row.get('Visitas Pendente') || '0').trim(), // CORRIGIDO
    ...
    for (const row of rows) {
      try {
        ...
        if (!box && (segmento === 'LJ COMPER' || segmento === 'LJ FORT')) {
          ...
          // Lê Visitas Pendente e converte para número
          const visitasPendenteRaw = String(row.get('Visitas Pendente') || '0').trim();
          const visitasPendenteNum = parseInt(visitasPendenteRaw) || 0; // CORRIGIDO: converte para número
          ...
          const item = {
            ...
            statusSep: String(row.get('Separação st') || '').trim(),
            statusConf: String(row.get('conf. St') || '').trim(),
            visitasPendente: visitasPendenteNum, // CORRIGIDO: agora é número
            prioridade: String(row.get('Prioridade') || '').trim()
          };
...
// ==================== FUNÇÃO getEstadoBoxes CORRIGIDA ====================
  ...
  // para quem for mexer nisso, saiba que estou com dor de cabeça ja de ver essas linhas!!  
