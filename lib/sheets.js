// lib/sheets.js - VERSÃO FUNCIONAL ORIGINAL
...
class SheetsService {
  ...
  async salvarNaBase(dados) {
    try {
      ...
      // Carrega todas as linhas da aba
      ...
      for (const linha of dados) {
        ...
        // Procura linha existente por matrícula (ou por nome, como fallback)
        const existente = rows.find(row =>
          row['Matricula'] === matricula || row['Nome'] === nome
        );
  
        if (existente) {
          // Atualiza apenas o status (e data, se quiser)
          existente['Status'] = status;
          existente['Data'] = hoje;
          await existente.save();
        } else {
          // Adiciona nova linha se não encontrada
          await sheet.addRow({
            'Supervisor': sup,
            'Aba': aba,
            'Matricula': matricula,
      ...
      return { ok: true, msg: 'Dados salvos/atualizados com sucesso' };
    } catch (error) {
