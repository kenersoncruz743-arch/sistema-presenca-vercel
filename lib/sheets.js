...
class SheetsService {
  ...
  /**
   * Remove colaborador do buffer buscando por Supervisor OU Grupo (aba)
   * @param {string} chave - Supervisor ou nome da Aba
  ...
  async removerBufferPorAba(chave, matricula) {
    try {
      ...
      const rows = await sheet.getRows();
      console.log(`[SHEETS] Buscando para remover: chave="${chave}", matricula="${matricula}"`);
  ...
  /**
   * Atualiza status do colaborador no buffer buscando por Supervisor OU Grupo (aba)
   * @param {string} chave - Supervisor ou nome da Aba
   * @param {string} matricula - Matrícula do colaborador
  ...
  async atualizarStatusBufferPorAba(chave, matricula, status) {
    try {
      ...
      const rows = await sheet.getRows();
      console.log(`[SHEETS] Buscando para atualizar status: chave="${chave}", matricula="${matricula}", status="${status}"`);
  ...
  /**
   * Atualiza desvio do colaborador no buffer buscando por Supervisor OU Grupo (aba)
   * @param {string} chave - Supervisor ou nome da Aba
   * @param {string} matricula - Matrícula do colaborador
   * @param {string} desvio - Novo desvio
   * @returns {Promise<{ok: boolean}>}
   */
  async atualizarDesvioBufferPorAba(chave, matricula, desvio) {
    try {
      ...
      const rows = await sheet.getRows();
      console.log(`[SHEETS] Buscando para atualizar desvio: chave="${chave}", matricula="${matricula}", desvio="${desvio}"`);
