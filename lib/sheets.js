...
class SheetsService {
  ...
  async init() {
    ...
    try {
      ...
      // Planilha principal (presença)
      this.doc = new GoogleSpreadsheet(process.env.GOOGLE_SHEETS_ID, serviceAccountAuth);
      await this.doc.loadInfo();
      console.log(`✅ Conectado à planilha de presença: ${this.doc.title}`);

      // Planilha de coletores
  ...
  async getSheet(sheetName, useColetores = false) {
    ...
    // Decide qual planilha usar
    const doc = useColetores ? this.docColetores : this.doc;
    
    if (!doc) {
      throw new Error('Planilha de coletores não configurada');
    }
  ...
  getDefaultHeaders(sheetName) {
    const headers = {
      // Headers da planilha de PRESENÇA
      ...
      // Headers da planilha de COLETORES
  ...
  // MÉTODOS DE PRESENÇA (usam planilha principal)
  async validarLogin(usuario, senha) {
    try {
      const sheet = await this.getSheet('Usuarios', false); // false = planilha principal
  ...
  async buscarColaboradores(filtro = '') {
    try {
      const sheet = await this.getSheet('Quadro', false);
      const rows = await sheet.getRows();
  ...
  async getBuffer(supervisor, aba) {
    try {
      const sheet = await this.getSheet('Lista', false);
      const rows = await sheet.getRows();
  ...
  async salvarNaBase(dados) {
    try {
      const sheet = await this.getSheet('Base', false);
  ...
  // MÉTODOS DE COLETORES (usam planilha separada)
  async obterDadosColaboradores() {
    try {
      const sheet = await this.getSheet('Quadro.1', true); // true = planilha de coletores
      const rows = await sheet.getRows();
  ...
  async salvarRegistro(dados) {
    try {
      ...
      const sheet = await this.getSheet('Historico', true); // true = planilha de coletores
      const rows = await sheet.getRows();
