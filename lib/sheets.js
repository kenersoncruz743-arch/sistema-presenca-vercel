...
class SheetsService {
  ...
  // SUBSTITUIR A FUNÇÃO limparColunasMapaCarga() no lib/sheets.js
  ...
  async limparColunasMapaCarga() {
    try {
      ...
      // ===== CORREÇÃO: Lista EXATA das colunas que PODEM ser limpas (SEM as protegidas) =====
      const colunasParaLimpar = [
        'Empresa',
        'SM',
        'Deposito',
        'BOX',
      ...
      // ===== COLUNAS PROTEGIDAS (NÃO DEVEM SER LIMPAS) =====
      const colunasProtegidas = [
        'Visitas Pendente',
        'Separação st',
        'conf. St',
        'Loja',
      ...
      colunasParaLimpar.forEach(col => {
        ...
        if (idx !== -1) {
          // Verifica se não está na lista de protegidas
      ...
      return { 
        ok: true, 
        msg: `${linhasLimpas} linhas limpas em ${indicesColunas.length} colunas! (${colunasProtegidas.length} colunas protegidas)`,
        total: linhasLimpas,
