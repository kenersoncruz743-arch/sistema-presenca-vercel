...
// Adicione estes métodos no lib/sheets.js após o método salvarNaBase()

async obterDadosColetores() {
  try {
    // Se não tem planilha separada, usa a mesma planilha (aba Quadro)
    const useColetores = !!process.env.GOOGLE_SHEETS_COLETORES_ID;
    const sheet = await this.getSheet('Quadro', useColetores);
    const rows = await sheet.getRows();
    
    console.log(`Obtendo dados de coletores: ${rows.length} registros`);
    
    const dados = [];
    for (const row of rows) {
      // Usa as colunas da sua planilha atual
    ...
    // Retorna dados mockados se der erro
    return [
      { chapa: '001', nome: 'Teste Colaborador', funcao: 'Operador' }
    ];
  }
}
...
async salvarRegistroColetor(dados) {
  try {
    ...
    if (!chapa || !numeroColetor) {
      return { ok: false, msg: 'Preencha todos os campos obrigatórios.' };
    }

    // Por enquanto, salva na aba Base da planilha principal
    const sheet = await this.getSheet('Base');
    ...
    await sheet.addRow({
      ...
      'Nome': nome,
      'Função': funcao,
      'Status': `Coletor ${numeroColetor} - ${tipoOperacao} - ${situacoesTexto}`,
...
async obterColetorStatus() {
  try {
    ...
    const mapa = {};
    for (let i = 1; i <= 145; i++) {
      // Simula alguns coletores com status
      if (i <= 10) {
        mapa[i] = {
